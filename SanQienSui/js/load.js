...
function initializeOverview(type_){
    ...
    $.getJSON('js/' + type_ + '/' + type_ +'.json', function(data){
		let title = '珍藏∕<a href="about_overview.html?type='+ type_ + '">' + data.title + '</a>'
		...
		if (data.has_class == 1) {
			data.class_list.forEach(element => {
				...
				project += '<a href="about_class.html?type='+ type_ +'&class='+ class_en +'">';
			...
			data.items.forEach(item => {
				project += '<a href="about_intro.html?type='+ type_ +'&id='+ item.id +'">';
				project += '<div class="col-m-6 col-4">'
				project += '<div class="shbox">'
				project += '<div class="imgbox">'
...
function initializeClass(class_, type_){
    ...
    $.getJSON('js/' + type_ + '/' + type_ +'.json', function(data){
		...
		let ch = '';
		items.forEach(item => {
			ch += '<a href="about_intro.html?type='+ type_ +'&class=' + class_ + '&id=' + item.id + '">';
		...
		$('#header').empty();
		let header = '<h1>珍藏∕<a href="about_overview.html?type='+ type_ +'">' + data.title +'</a>∕';
		header += '<a href="about_class.html?type='+ type_ +'&class=' + class_ + '">'+ classZh + '</a></h1>';
...
function introNoClass(id_, type_){
	...
	$.getJSON('js/' + type_ + '/' + type_ + '.json', function(data){
		...
		$('#header').empty();
		let header = '<h1>珍藏∕<a href="about_overview.html?type='+ type_ +'">' + data.title + '</a>∕';
		header += '<a href="about_intro.html?type='+ type_ + '&id=' + id_ + '">' + item_.name + '</a></h1>';
		$('#header').append(header);
	...
	$.getJSON('js/' + type_ + '/' + type_ + '.json', function(data){			
		data.class_list.forEach(element => {
			if (class_ == element.class.en) {
				...
				$('#header').empty();
				let header = '<h1>珍藏∕<a href="about_overview.html?type='+ type_ +'">' + data.title + '</a>∕';
				header += '<a href="about_class.html?type='+ type_ +'&class=' + class_ + '">'+ classZh + '</a>∕';
				header += '<a href="about_intro.html?type='+ type_ +'&class=' + class_ + '&id=' + id_ + '">' + item_.name + '</a></h1>';
				$('#header').append(header);
